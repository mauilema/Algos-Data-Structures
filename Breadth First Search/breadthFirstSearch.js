class Node {
    ...
    breadthFirstSearch(array) { // this class method takes in an empty array as the input argument. Eventually, this array will store all the node names in alphabetical order.
       ...
       const currentNode = queue.shift(); //declare a const variable "currentNode". It will store the current node that was just shifted from the left beginning of the array. The shift
           //method is used to get the effect of a queue FIFO mechanism.
       ...
       for (const child of currentNode.children) { //use a for off loop to access each child belonging to the current node (the one just shifted off of the queue). currentNode.children
           ...
           queue.push(child) //push each of the current node's child into the queue. The leftmost child will be shifted off of the array in the next step which is the first line
           //in this while loop. 
       }	 
       }
