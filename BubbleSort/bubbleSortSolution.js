function bubbleSort(array) {
	...
	while (!sorted) {  //while !sorted is truthy run the code block inside the while loop
		sorted = true; //changes sorted's value from false to true. Once all for loops iterations/swapping are done,
		...
		for (let i = 0; i < array.length-1-lastElCounter; i++) { //loop through the array, stop at the last
			...
			if (array[i] > array[i+1]){//if the value of the element is greater than the value of the element 
	...
	return array;//once all iterations/swapping are done, return the final sorted array
}

function swapEl (i, j, array) {//takes both indexes to be swapped and the array as arguments. Uses new ES6 syntax to swap 
	//those elements in the array
